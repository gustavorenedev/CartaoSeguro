...
namespace CartaoSeguro.Tests.Application.Services.Auth;
...
public class AuthServiceTests
{
    ...
    public async Task RegisterAsync_ShouldReturnCreatedUserResponse_WhenValidDataIsProvided()
    {
        ...
        userRepositoryMock.Setup(repo => repo.AddAsync(It.IsAny<Domain.User.User>())).ReturnsAsync(newUser);

        var mapperMock = new Mock<IMapper>();
        mapperMock.Setup(mapper => mapper.Map<CreateUserResponse>(It.IsAny<Domain.User.User>()))
    ...
    public async Task RegisterAsync_ShouldThrowException_WhenEmailIsAlreadyInUse()
    {
        ...
        userRepositoryMock.Setup(repo => repo.GetByEmailAsync(createUserRequest.Email))
                          .ReturnsAsync(new Domain.User.User());
